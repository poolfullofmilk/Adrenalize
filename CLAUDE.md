...
There is no solution file — build the `.csproj` directly.
...
Run both of these after every change, before reporting the work as done. They are not optional and they are not covered by `dotnet build`.
...
**RazorStyle.ps1** at the repository root checks `.razor` files against the razor conventions below. It only reports, it never rewrites, and it exits 1 on any violation:

```
powershell -NoProfile -ExecutionPolicy Bypass -File .\RazorStyle.ps1
```

Use `pwsh` instead of `powershell` where PowerShell 7 is installed; this machine only has Windows PowerShell 5.1, which the script supports. It defaults to scanning the whole repository, and takes `-Path` to narrow the scan and `-Rule` to filter to one rule.

This repository currently contains no `.razor` files, so the script reports `Checked 0 razor file(s)`. It lives here so the convention travels with the repo and is already wired up if any Blazor UI is ever added. Do not delete it as dead weight.
...
4. `GameScanner.ScanInstalledGameProcessNames()` walks the disk once and returns a `Dictionary<processName, displayName>`.
...
| `Configuration/AppConfig.cs` | Constants only, no logic |
...
| `Utilities/Logger.cs` | Timestamped colored console output |

## Design Decisions

These are the non-obvious calls. Do not undo them without a reason.

**Services are controlled through WMI, not `sc.exe`.** `System.Management` is already a dependency because service discovery needs `Win32_Service`. Calling `InvokeMethod("StopService")` on the same objects means no process spawning at all. The previous version shelled out to `sc.exe` for every stop, start, and state poll — roughly 60 process launches per reset — and scraped the text output for `RUNNING` and `STOPPED`. If you need something WMI cannot do, prefer `ServiceController` over shelling out again.
...
**Executable picking is a score, not a rule.** A game folder usually holds several `.exe` files. `ScoreExecutable` rewards names containing `win64` or `shipping` and names matching the folder, and penalizes `launcher`, `helper`, `crash`, `report`, `uninstall`, and `setup`. The highest score wins. `s_executableRejectTokens` then vetoes the winner outright if it is clearly a helper. These weights are tuning against real installs, not a general algorithm — adjust them when a game is detected wrongly, and add a case to the self-check.
...
Reach for the standard library and the platform before writing code, and before adding a package. The two existing dependencies both earn their place: `System.Management` for WMI service control, `TaskScheduler` because the logon-without-UAC requirement cannot be met any other way.

Verify with `dotnet build`, the `--selftest` run, `csharpier format .`, and `RazorStyle.ps1`. All four, every time. Anything beyond parsing and scoring needs a real machine with AMD hardware and installed games, so state plainly what you did and did not verify.
