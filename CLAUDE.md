...
A single-project .NET 10 Windows console application that keeps AMD Adrenalin healthy: it repairs the AMD stack when it breaks, and gives one tray action that fixes it by hand.

The problem it solves: AMD's services and Adrenalin itself fall over, usually shortly after a game exits, and once they have the only way back used to be killing every AMD process in Task Manager. This watches for that and repairs it, and keeps AMD's Record And Stream capture switched off, because that switch is what injects AMD's hook into every game.

It started life as the opposite: a tool that restarted Adrenalin after every game launch so the overlay would attach. That behaviour was removed in 2.3. With the overlay, metrics and recording all off there is nothing to attach, and restarting AMD underneath a running game killed the host of the hook that was still loaded in that game, which is the most likely reason AMD then broke on exit.
...
5. Console input is locked down, then WMI event watchers subscribe to process exits and AMD service state. A dead AMD service triggers a repair, a watched game exiting triggers a health check 30 seconds later that repairs only if something is actually broken, and Adrenalin exiting on its own is simply started again. Nothing happens when a game starts.
...
**Nothing restarts AMD when a game starts.** A reset kills `AMDRSServ`, the Radeon Settings Host Service, and that process is the host of `amdihk64.dll`, the hook AMD injects into every game. Killing it underneath a running game leaves the game holding a hook whose host has been replaced, and the stack reliably broke when that game then exited. The start reset existed to make the overlay attach; with the overlay, metrics and recording off there is nothing to attach. Resets now come from four places only: an AMD service dying, a game exit that leaves AMD unhealthy, a broken stack one minute after launch, and the tray Reset. A healthy stack is always left alone, so closing a game normally logs one grey line and does nothing.
...
**Record And Stream is forced off.** `EnsureCaptureDisabled` sets `DvrEnabled` to 0 under the current user's AMD DVR key at every app start and before every Adrenalin launch. That single switch makes AMD inject its capture hook and hardware encoder into every game even with the overlay, metrics, hotkeys and instant replay all turned off, and on this machine it sat next to Medal's own capture hook in the same game. Adrenalin only reads the value when it starts, which is why it is written before each launch rather than once. Driver updates have been known to restore AMD defaults, so enforcing it is deliberate; a user who wants AMD recording turns it back on in Adrenalin and removes this call.

**Resets never run on the thread that asked for one.** `TryTriggerResetAsync` hands `ExecuteReset` to `Task.Run`. A reset takes around forty seconds, and running it inline froze whichever thread triggered it: the tray's message pump for a manual Reset, which Windows then reported as Adrenalize not responding, or a WMI delivery thread for a crash repair.
...
**The console accepts no input.** `DisableConsoleInput` clears `ENABLE_ECHO_INPUT` and `ENABLE_LINE_INPUT` on the standard input handle, so keystrokes neither echo nor form lines. There is no command parser, every action is a tray menu item. Do not add console commands back, that split meant the same settings were written in two places. The same call also clears `ENABLE_QUICK_EDIT_MODE`, which needs `ENABLE_EXTENDED_FLAGS` set to take effect. With QuickEdit on, a single click inside the window starts a text selection, and while a selection exists every write to the console blocks. The app logs from every watcher, so one stray click froze the whole thing, and with the window hidden in the tray the selection was never cleared. The window title reads Select Adrenalize when this has happened. `GetConsoleMode` fails when standard input is redirected, which is why the call is guarded rather than asserted. `--selftest` deliberately does not call `DisableConsoleInput`, the mode change is not restored on exit, so a self-check run from an interactive terminal would leave that terminal with echo off.
